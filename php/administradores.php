...
if ($num_filas_usuarios == 0){
    ...
    echo"<table border = '1'>
        ...
        while($fila = obtener_resultados($usuarios)){
            extract($fila);
            echo "<tr><td>$id_usuario</td><td>$pass</td><td>$nombre</td><td>$apellido</td><td>$dni</td><td>$email</td><td>$telefono</td><td>$tipo</td></tr>";
        }
        echo "</table>";
}
...
$_POST['nuevo_telefono'], $_POST['nuevo_tipo'])) {
    ...
    if (empty($id_usuario) ||empty($nueva_pass) ||empty($nuevo_nombre) ||empty($nuevo_apellido) ||empty($nuevo_dni) ||empty($nuevo_email) ||empty($nuevo_telefono) || $nuevo_tipo === '') {
        ...
        modificar_usuario($con, $id_usuario,$nueva_pass,$nuevo_nombre,$nuevo_apellido,$nuevo_dni,$nuevo_email,$nuevo_telefono, $nuevo_tipo);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pass'], $_POST['nombre'], $_POST['apellido'],$_POST['dni'],$_POST['email'],$_POST['telefono'], $_POST['tipo'])) {
    $pass = $_POST['pass'];
    $nombre = $_POST['nombre'];
    ...
    if (empty($id_usuario) ||empty($pass) ||empty($nombre) ||empty($apellido) ||empty($dni) ||empty($email) ||empty($telefono) || $tipo === '') {
        echo "<p style='color: red;'>Error: Todos los campos son obligatorios.</p>";
    }else{
          crear_usuarios($con,$pass,$nombre,$apellido,$dni,$email,$telefono,$tipo);
